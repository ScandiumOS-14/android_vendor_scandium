...
func init() {
	android.RegisterModuleType("scandium_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = scandiumExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = scandiumExpandVariables(ctx, dep_file)
	...
	cmd := scandiumExpandVariables(ctx, String(g.properties.Cmd))
